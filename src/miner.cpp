...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("UMIMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("UMIMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("UMIMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("umi-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("UMIMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running UMIMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadUMIMiner() exception");
    } catch (...) {
        LogPrintf("ThreadUMIMiner() exception");
    }

    LogPrintf("ThreadUMIMiner exiting\n");
}
